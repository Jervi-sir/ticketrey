...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        <div class="min-h-screen bg-gray-100">
            ...
            <!-- Page Content -->
            <main>
                <h1 class="text-center py-3 text-purple-400 font-bold">All Advertisers</h1>
                    ...
                    <table class="table-auto mx-auto">
                      ...
                      <tbody>
                        ...
                        <tr class="@if($loop->iteration % 2 == 0)bg-blue-200 @endif">
                            <td class="border border-blue-500 px-4 py-2 text-blue-600 font-medium text-center">
                                <form action="{{ route('admin.editAdv', ['id'=>$adv->id]) }}" method="GET">
                                    @csrf
                                    <button type="submit" class="rounded-full bg-red-300 text-white px-4 py-1 hover:bg-gray-800 ease-in-out duration-200">Edit</button>
                                </form>
                                <form action="{{ route('admin.confirmAdv', ['id'=>$adv->id]) }}" method="POST">
                                    @csrf
                                    @if ($adv->is_verified == 0)
                                    ...
                                    @else
                                    <button type="button" class="rounded-full  text-green-500 px-4 py-1 hover:bg-gray-800 ease-in-out duration-200" disabled>Verified</button>
                                    @endif
                                </form>
                            </td>
                            <td class="border border-blue-500 px-4 py-2 text-blue-600 font-medium">{{ $adv->is_verified == 1 ? 'true' : 'false' }}</td>
